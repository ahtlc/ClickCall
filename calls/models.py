...
class Call(models.Model):
  ...
  hour = models.TimeField(auto_now = True, verbose_name = "Hora da Ligação")
  status = models.CharField(max_length = 256, choices = STATUS_CHOICES, default = IN_PROGRESS, verbose_name = "Status")
  ...
  def __str__(self):
    return self.contact.name.capitalize() + " " + datetime.datetime.strftime(self.date, "%d/%m/%Y")
...
class Subject(models.Model):
  ...
  def __str__(self):
    return self.name.capitalize()
>>>>>>> hotfix(call-history) add status options in models
